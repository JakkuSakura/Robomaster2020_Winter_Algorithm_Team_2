...
class GlobalPathSpecifier{
 public:
  GlobalPathSpecifier(ros::NodeHandle& given_nh):
    ...
    setpoint_pub_ = nh.advertise<visualization_msgs::Marker>("/global_planner/set_point", 10);
    global_path_pub_ = nh.advertise<nav_msgs::Path>("/global_planner/path", 10);
...
{
  ros::init(argc, argv, "global_path_specifier");
  ros::NodeHandle nh("~");
  GlobalPathSpecifier global_planner(nh);
  ros::spin();
  return 0;
}
